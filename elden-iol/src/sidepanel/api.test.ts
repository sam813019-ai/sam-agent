...
describe('recognizeImage', () => {
  it('回傳通過 schema 驗證的辨識結果', async () => {
    ...
    await expect(recognizeImage(fakeFile(), MASKS)).resolves.toMatchObject({ device: 'IOLMaster700' });
  });
  ...
  it('上傳的是遮蔽後的圖，不是原檔', async () => {
    ...
    await recognizeImage(fakeFile(), MASKS);
  ...
  it('送出的內容不含檔名等原始檔案資訊', async () => {
    ...
    vi.stubGlobal('fetch', spy);
    await recognizeImage(new File([new Uint8Array([1])], '林許謹_病歷12345.jpg', { type: 'image/jpeg' }), MASKS);
  ...
  it('伺服器回錯誤時，把錯誤訊息拋出來給 UI 顯示', async () => {
    ...
    await expect(recognizeImage(fakeFile(), MASKS)).rejects.toThrow('辨識服務忙碌中');
  });

  it('回應不符合 schema 時拋錯，不讓壞資料流進 UI', async () => {
    mockFetch({ device: 'IOLMaster700' }, 200);
    await expect(recognizeImage(fakeFile(), MASKS)).rejects.toThrow();
  });
});
