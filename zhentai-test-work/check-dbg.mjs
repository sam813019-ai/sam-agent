...
const CHECKS = [
  ...
  },
  {
    ...
    page: 'index',
    fn: async (page) => {
      // observer 門檻 threshold:.4，要各自捲到才會觸發
      await page.evaluate(() => document.querySelector('#about .ab-bignum').scrollIntoView({ block: 'center' }));
      await page.waitForTimeout(2200);
      console.log('DBG scrollY=', await page.evaluate(()=>window.scrollY),
                  'rectTop=', await page.evaluate(()=>Math.round(document.querySelector('#about .ab-bignum').getBoundingClientRect().top)),
                  'innerH=', await page.evaluate(()=>window.innerHeight),
                  'modalOpen=', await page.evaluate(()=>!!document.querySelector('.jt-modal.open')));
      const big = await page.$eval('#about .ab-bignum .count', (e) => e.textContent.trim());
      if (big === '0') throw new Error('大數字 count-up 沒跑');

      await page.evaluate(() => document.querySelector('#about .stats').scrollIntoView({ block: 'center' }));
      await page.waitForTimeout(2200);
      const vals = await page.$$eval('#about .stats .count', (els) => els.map((e) => e.textContent.trim()));
      if (!vals.length) throw new Error('找不到 .stats .count 元素');
      if (vals.some((v) => v === '0')) throw new Error(`數據區 count-up 沒跑完，值為 ${vals.join(', ')}`);
    },
  },
  {
